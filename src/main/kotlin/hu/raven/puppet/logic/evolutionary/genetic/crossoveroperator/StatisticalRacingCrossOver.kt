...
class StatisticalRacingCrossOver : CrossOverOperator {
    ...
    override fun <S : ISpecimenRepresentation> invoke(
        ...
        synchronized(iterationLock) {
            newIteration = iteration / 5 != algorithm.iteration / 5
            if (newIteration)
        ...
        if (newIteration) {
            actualStatistics?.let { statistics ->
                synchronized(statistics) {
                    statistics.run = (statistics.run + algorithm.population.size) * 8 / 10
        ...
        operator?.let { operator ->
            actualStatistics?.let { actualStatistics ->
                ...
                synchronized(actualStatistics) {
                    ...
                    actualStatistics.success += (algorithm.iteration - parents.first.iteration) / child.cost /
                            (parents.first.orderInPopulation + 1).toDouble().pow(2)

                }
                    /*else*/ if (parents.second.cost > child.cost) {
                    actualStatistics.success += (algorithm.iteration - parents.second.iteration) / child.cost /
